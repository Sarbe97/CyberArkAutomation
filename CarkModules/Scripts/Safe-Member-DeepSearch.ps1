...
try {
    ...
    if ($memberSearch) {
        ...
        Write-Host "[INFO] No member filter applied. Retrieving all members." -ForegroundColor Cyan
    }
    ...
    foreach ($safe in $safeNames) {
        ...
        try {
            ...
            # Filter members by memberName if applicable
            if ($memberSearch) {
                $members = $members | Where-Object { $_.memberName -like "*$memberSearch*" }
                Write-Host "[INFO] After filtering, $($members.Count) member(s) match the search." -ForegroundColor Cyan
            }
