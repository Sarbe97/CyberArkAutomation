...
. (Join-Path $FeatureRoot   "Modules\SVC_SharePointReport.ps1")
...
try {
    ...
    if ($null -ne $cfgSP -and $cfgSP.Enabled -and $null -ne $config.SharePoint) {
        Write-Log -Message "========== PHASE 4: SHAREPOINT UPLOAD ==========" -ScriptName $ScriptName -LogPath $LogPath
        Publish-SVCSharePointReport `
            -GlobalConfig            $config `
